...
from analysis.claude_cli import run_extraction, CLIResult
...
def _doc_to_text(row: asyncpg.Record) -> str:
    parts = []
    if row["kind"]:
...
async def _persist(pool: asyncpg.Pool, *, document_id: str, model: str, result: CLIResult) -> None:
    ...
    async with pool.acquire() as c:
        async with c.transaction():
            await c.execute(
                ...
                "ok" if so and not result.is_error else "failed",
...
async def run(*, limit: int, source: str | None, since: str | None, model: str, concurrency: int) -> None:
    dsn = os.environ.get("DATABASE_URL", "postgresql://reddit:reddit@localhost:5432/reddit")
    pool = await asyncpg.create_pool(dsn, min_size=1, max_size=4)
    try:
        async with pool.acquire() as c:
            rows = await c.fetch(SELECT_PENDING, PROMPT_VERSION, EXTRACTOR, source, since, limit)
        if not rows:
            log.info("extract.nothing_pending")
            return

        log.info("extract.pending", n=len(rows), model=model, concurrency=concurrency)
        ...
        async def process(row: asyncpg.Record) -> None:
            ...
            async with sem:
                doc_id = row["id"]
                text = _doc_to_text(row)
                try:
                    res = await run_extraction(text, model=model)
                ...
                if res.is_error or res.structured_output is None:
                    ...
                    return
                await _persist(pool, document_id=doc_id, model=model, result=res)
                ok += 1
